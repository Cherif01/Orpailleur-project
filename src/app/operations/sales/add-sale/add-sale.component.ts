...
export class AddSaleComponent implements OnInit {
    ...
    this.serviceLot.getList('lot', 'readNotExpedier.php').subscribe({
      next: (arrivage: any) => {
        ...
        arrivage.forEach((lot: any) => {
          if (lot.poids_restant > 0) this.TabLot.push(lot);
        });
        // }
      },
    });
      ...
      .subscribe({
        next: (data: any) => {
          ...
          if (data.length > 0) {
            ...
            this.message =
              "Aucun achat restant pour l'expedition dans ce lot !";
          }
        },
      });
  }
  ...
  sendExpedition(): void {
    ...
    selected.forEach((v) => {
      ...
      formExpeditionData.append(
        'id_expedition',
        this.expditionObj.id_expedition
      );
  ...
  // ENVOIE DE L'EXPEDITION
  sendExpeditionByPOIDS(): void {
    const objetSendbyAchat = {
      id_expedition: this.expditionObj.id_expedition,
      idLot: this.expditionObj.lot_exp,
      idAchat: '',
      idItem: '',
      tableName: 'table_expedition',
    };

    console.log(objetSendbyAchat);

    // this.service
    //   .create(
    //     'client',
    //     'expedition.php',
    //     convertObjectInFormData(objetSendbyAchat)
    //   )
    //   .subscribe({
    //     next: (response: any) => {
    //       console.log('RES : ', response);
