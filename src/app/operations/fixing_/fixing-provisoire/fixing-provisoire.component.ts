...
@Component({
  selector: 'app-fixing-provisoire',
  templateUrl: './fixing-provisoire.component.html',
  styleUrls: ['./fixing-provisoire.component.css']
})
export class FixingProvisoireComponent implements OnInit {
  // FORM
  Fixing = new FormGroup({
    idFournisseur: new FormControl('', [Validators.required]),
    poidsFixer: new FormControl('', [Validators.required]),
    carratMoyen: new FormControl('', [Validators.required]),
    fournisseur: new FormControl(null),
    fixingBourse: new FormControl('', [Validators.required]),
    discompte: new FormControl('', [Validators.required]),
    created_by: new FormControl(1)
  })

  id_session = localStorage.getItem('id_session')

  title = 'Fixing Provisoire'
  // Control
  ...
  fixingPOST (form: FormGroup): void {
    ...
    if (form.valid) {
      ...
      formData.append('carratMoyen', form.value.carratMoyen)
      formData.append('fixingBourse', form.value.fixingBourse)
      formData.append('discompte', form.value.discompte)

      this.service.create('fixing', 'addProvisoire.php', formData).subscribe({
        next: (reponse: any) => {
