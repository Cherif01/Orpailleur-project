...
export class RapportFournisseurComponent implements OnInit {
  ...
  goRapport(nomClomplet: string, id: number): any {
    ...
    this.service.getUnique('fournisseur', 'operation.php', id).subscribe({
      next: (data: any) => {
        ...
        // Inverser l'ordre des objets dans fusionTab
        this.fusionTab.reverse();

        // Afficher les 5 premières lignes initialement
        this.displayedRows = this.fusionTab.slice(0, 5);
        this.SoldeUSD = format2Chart(solde);
        // console.log("FUSION : ", this.fusionTab);
        // console.log("FUSION 2: ", this.fusionTab.reverse());
  ...
  loadMore(): void {
    this.showAll = true;
    this.displayedRows = this.fusionTab;
  }
