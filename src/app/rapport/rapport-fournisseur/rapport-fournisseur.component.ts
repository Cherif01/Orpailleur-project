...
export class RapportFournisseurComponent implements OnInit {
  ...
  displayedColumns2 = ['Date', 'Arrivage', 'Fournisseur', 'Poids_total', 'Carrat_moyen'];
  ...
  goRapport(nomClomplet: string, id: number): any {
    ...
    this.service.getUnique('fournisseur', 'operation.php', id)
      .subscribe({
        next: (data: any) => {
          ...
          // CONVOIE
          this.service.LIST_BY_ID('rapport', 'fournisseur/convoie.php', id.toString())
            .subscribe({
              next: (data: any) => {
          ...
          // Item
          this.service.LIST_BY_ID('rapport', 'fournisseur/getItembydate.php', id.toString())
            .subscribe({
              next: (data: any) => {
                // console.log("Data : ", data);
