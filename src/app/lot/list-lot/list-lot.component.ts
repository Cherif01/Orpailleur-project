...
export class ListLotComponent implements OnInit {
  ...
  listLotEvent(idLotSelect: any) {
    ...
    this.serviceLot
      .getOneByIdSimple('lot', 'item_by_lot.php', idLotSelect)
      .subscribe({
        next: (data: any) => {
          // console.log(data);
          this.dataSource_maj.data = data;
          this.dataSource_maj.paginator = this.paginator;
        },
      });
  }
  ...
  saveTableData(element: any) {
    ...
    this.serviceLot.UpdateItem('achat', 'updateItem.php', objetForm).subscribe({
      next: (response) => {
