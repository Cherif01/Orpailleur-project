...
export class CaissePrincipaleComponent implements OnInit, AfterViewInit {
  ...
  // DUPLICATION
  @ViewChild('elementADupliquer', { static: false }) elementADupliquer:
    | ElementRef
    | any;
  ...
  amount: number = 0; // Montant en USD
  rate: number = 0; // Taux d'échange
  result: number = 0; // Résultat en GNF
        ...
        if (result?.event && result.event === 'insert') {
          ...
          switch (form.operation) {
            ...
            case 3:
              ...
              // Entree de caisse
              this.service
                .LIST_ALL_PRECIS('caisse', 'solde.php')
                .subscribe((v) => {
                  if (form.devise == 1) {
                    // GNF
                ...
                .subscribe((v) => {
                  if (form.devise == 1) {
                    if (v[0].soldeGNF < form.montant) {
                      this.snackBar.open(
                        'Operation superieur au solde actif...? !',
                        "D'accord !",
                        {
                          duration: 3000,
                  ...
                  if (form.operation == 4 && form.fournisseur == null) {
                    this.snackBar.open(
                      'Aucun fournisseur selectionnez..? !',
                      "D'accord !",
                      {
                        duration: 3000,
                        horizontalPosition: 'right',
  ...
  getInfo(id: any): void {
    this.service
      .getOneById('public', 'getOne.php', id, 'table_caisse')
      .subscribe({
        next: (data: any) => {
          ...
          if (data.operation == 3 || data.operation == 4) {
            this.service
              .getUnique('fournisseur', 'profile.php', data.fournisseur)
              .subscribe({
                next: (f: any) => {
                  // console.log("frs", f);
  ...
  deleteFunction(idCaisse: any) {
    ...
    this.dialog
      ...
      .subscribe((data) => {
        if (data) {
          // console.log(data);
          this.service.deleteByID('caisse', 'delete.php', idCaisse).subscribe({
            next: (value) => {
  ...
  getCaisse(): void {
    ...
    if (this.rechercheIntervalleForm.valid) {
      ...
      this.service
        ...
        .subscribe({
          next: (data: any) => {
            console.log('Caisse : ', data[0]);
            this.dataSource.data = data[0];
            this.soldeUSD = data[1];
  ...
  solde_USD = 0;
  getSoldeAll() {
    this.service.LIST_ALL_PRECIS('caisse', 'solde.php').subscribe({
      next: (result) => {
  ...
  salaire = 0;
  statMensuelle() {
    this.service.LIST_ALL_PRECIS('caisse', 'stats_mensuel.php').subscribe({
      next: (result: any) => {
        console.log(result);
        this.extra = result.extra;
        this.facture = result.facture;
        this.salaire = result.salaire;
      },
      error: (error) => {
        // console.error(error);
      },
  ...
  createSecteurChart() {
      new Chart('polarChart', {
        type: 'bar',
        data: {
          labels: ['Depenses-extra', 'Facture Payer', 'Salaire du personnel'],
          datasets: [{
            data: [this.extra ?? 1, this.facture ?? 1, this.salaire ?? 1],
            backgroundColor: ['green', 'orange', 'blue'], // Vert et Rouge
            // hoverOffset: 10
          }]
        },
        options: {
