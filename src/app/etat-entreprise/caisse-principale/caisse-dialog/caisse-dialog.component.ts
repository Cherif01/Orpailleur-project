...
export class CaisseDialogComponent implements OnInit {
  ...
  currentDateTime(): string {
    const now = new Date();
    // Format ISO 8601 pour le type "datetime-local" : 'YYYY-MM-DDTHH:mm'
    const year = now.getFullYear();
    const month = String(now.getMonth() + 1).padStart(2, '0'); // Mois au format 'MM'
    const date = String(now.getDate()).padStart(2, '0'); // Jour au format 'DD'
    const hours = String(now.getHours()).padStart(2, '0'); // Heures au format 'HH'
    const minutes = String(now.getMinutes()).padStart(2, '0'); // Minutes au format 'mm'
