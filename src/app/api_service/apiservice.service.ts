...
export class ApiserviceService {
  ...
  UpdateItem(api: string, suffixUrl: string, data: any) {
    ...
    return this.http.put<any[]>(
      `${BASE_URL}${api}/${suffixUrl}/${data.id}/`,
      data
    );
  }
  ...
  getFacture(api: string, suffixUrl: string, data: any[]) {
    const url = `${BASE_URL}${api}/${suffixUrl}`;
    // console.log("📡 Envoi à l'API :", JSON.stringify({ data }));  // Vérifie les données envoyées
    // Envoie directement les idFixing en tant que tableau
