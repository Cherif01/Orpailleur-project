...
export class FactureventeComponent implements OnInit, AfterViewInit {
  ...
  EnvoieFixingDetail (tabFixingValidInitAndItemsSelect: any) {
    ...
    tabFixingValidInitAndItemsSelect[1].forEach((insert: any) => {
      ...
      formData.append(
        'idFournisseur',
        tabFixingValidInitAndItemsSelect[0][0].idFournisseur
      )
      formData.append(
        'idFixing',
        tabFixingValidInitAndItemsSelect[0][0].idFixing
      )
      formData.append(
        'codeGenerate',
        tabFixingValidInitAndItemsSelect[0][0].codeGenerate
      )
      ...
      this.service.create('fixing', 'addTableDetail.php', formData).subscribe({
        next: (response: any) => {
          ...
          // Update
          this.service
            .Update('fixing', 'updateItem.php', updateFormData)
            .subscribe({
              next: responseUpdate => {
                ...
                // if (
                //   this.format2Chart(this.poids_fixer) ==
                //   this.format2Chart(this.PVendu)
                // ) {
                // console.log("cloture en cours...");
                const updateFormData2 = new FormData()
                updateFormData2.append(
                  'id',
                  tabFixingValidInitAndItemsSelect[0][0].idFixing
                )
                ...
                // Update
                this.service
                  .Update('fixing', 'clotureFixing.php', updateFormData2)
                  .subscribe({
                    next: responseUpdate => {
                      // console.log(
                      //   'Fixing cloturer et redirection en cours... : ',
                      //   responseUpdate
                      // )
                      this.router.navigate([
                        '/operation/facture-fournisseur/' + this.ID_fournisseur
                      ])
                    },
                    error: err => {
  ...
  hideSelectedItem (items: any[]) {
    let tab = this.listItems.filter(
      v => !items.find(item => item.idItem === v.id)
    )
  ...
  // GET
  getFixing (): void {
    this.service.getUnique('fixing', 'getOne.php', this.IDFixing).subscribe({
      next: data => {
        ...
        this.service
          .getUnique('fixing', 'listAchat.php', this.ID_fournisseur)
          .subscribe({
            next: data => {
  ...
  logSelected () {
    ...
    let poidsT: number = 0
    let codeGenerate: any =
      Math.floor(Math.random() * (1000000 - 1000 + 1)) + 2023
    let TabDonnees: any[] = []
    let TabItems: any[] = []
    ...
    if (
      this.format2Chart(poidsT) <=
      this.format2Chart(this.poids_fixer - this.poidsAttribuer)
    ) {
      this.EnvoieFixingDetail(TabData)
    } else {
      this.snackBar.open(
        'Le formattage a retourner une erreur, verifier svp !!!',
        "D'accord !",
        {
          duration: 5000,
          horizontalPosition: 'right',
  ...
  viewDetailAchat (idAchat: any): void {
    ...
    this.service.getUnique('fixing', 'getItemFalse.php', idAchat).subscribe({
      next: (items: any) => {
        ...
        this.cMoyenActif = this.format2Chart(
          this.carrat_moyen / this.PT_Restant
        )
  ...
  UFixing (form: FormGroup): void {
    if (form.valid) {
      ...
      this.serviceVendor.mettreAJourRessource(form.value).subscribe({
        next: response => {
      ...
      if (this.poids_fixer - this.poidsAttribuer >= form.value.poids_select) {
        ...
        this.service.create('fixing', 'addTablePoids.php', formData).subscribe({
          next: (response: any) => {
            ...
            // if (
            //   this.poids_fixer ==
            //   this.poidsAttribuer + form.value.poids_select
            // ) {
            // console.log("cloture en cours...");
            const updateFormData = new FormData()
            ...
            this.service
              .create('fixing', 'addTableDetail.php', formData)
              .subscribe({
                next: (response: any) => {
                  // console.log("RES : ", response);
                  // Update
                  this.service
                    .Update('fixing', 'clotureFixing.php', updateFormData)
                    .subscribe({
                      next: responseUpdate => {
                        // console.log("responseUpdate : ", responseUpdate);
                        this.router.navigate([
                          '/operation/facture-fournisseur/' +
                            this.ID_fournisseur
                        ])
                      },
                      error: err => {
        ...
        this.snackBar.open(
          'Poids superieur au reste a valider !',
          "D'accord !",
          {
            duration: 3000,
            horizontalPosition: 'right',
